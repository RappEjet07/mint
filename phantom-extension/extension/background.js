 ...
 *
 * Handles keypair generation, storage, and signing operations.
 * Uses ed25519 from TweetNaCl (embedded below for simplicity).
 */

// ============================================================
// Embedded TweetNaCl (minimal ed25519 + random)
// ============================================================
const nacl = (function () {
  "use strict";

  // We'll use a simplified approach: store the raw seed (32 bytes),
  // and derive the keypair using the Web Crypto API with Ed25519.
  // However, Chrome doesn't support Ed25519 in Web Crypto yet in all versions,
  // so we embed a minimal TweetNaCl sign implementation.

  // --- TweetNaCl sign-only port ---
  ...
  function crypto_hashblocks_hl(hh, hl, m, n) {
    ...
    while (n >= 128) {
      ...
      for (i = 0; i < 80; i++) {
        ...
        if (i % 16 === 15) {
          for (j = 0; j < 16; j++) {
            ...
            var s1h =
              ((wh_j14 >>> 19) | (wl_j14 << 13)) ^
              ((wl_j14 >>> (61 - 32)) | (wh_j14 << (32 - (61 - 32)))) ^
              (wh_j14 >>> 6);
            var s1l =
              ((wl_j14 >>> 19) | (wh_j14 << 13)) ^
              ((wh_j14 >>> (61 - 32)) | (wl_j14 << (32 - (61 - 32)))) ^
              ((wl_j14 >>> 6) | (wh_j14 << 26));

            a =
              (l & 0xffff) + (sl & 0xffff) + (s1l & 0xffff) + (wl_j9 & 0xffff);
            b = (l >>> 16) + (sl >>> 16) + (s1l >>> 16) + (wl_j9 >>> 16);
            c =
              (h & 0xffff) + (sh & 0xffff) + (s1h & 0xffff) + (wh_j9 & 0xffff);
            d = (h >>> 16) + (sh >>> 16) + (s1h >>> 16) + (wh_j9 >>> 16);
      ...
      h = ah0;
      l = al0;
      a = (l & 0xffff) + (hl[0] & 0xffff);
      b = (l >>> 16) + (hl[0] >>> 16);
      c = (h & 0xffff) + (hh[0] & 0xffff);
      d = (h >>> 16) + (hh[0] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah1;
      l = al1;
      a = (l & 0xffff) + (hl[1] & 0xffff);
      b = (l >>> 16) + (hl[1] >>> 16);
      c = (h & 0xffff) + (hh[1] & 0xffff);
      d = (h >>> 16) + (hh[1] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah2;
      l = al2;
      a = (l & 0xffff) + (hl[2] & 0xffff);
      b = (l >>> 16) + (hl[2] >>> 16);
      c = (h & 0xffff) + (hh[2] & 0xffff);
      d = (h >>> 16) + (hh[2] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah3;
      l = al3;
      a = (l & 0xffff) + (hl[3] & 0xffff);
      b = (l >>> 16) + (hl[3] >>> 16);
      c = (h & 0xffff) + (hh[3] & 0xffff);
      d = (h >>> 16) + (hh[3] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah4;
      l = al4;
      a = (l & 0xffff) + (hl[4] & 0xffff);
      b = (l >>> 16) + (hl[4] >>> 16);
      c = (h & 0xffff) + (hh[4] & 0xffff);
      d = (h >>> 16) + (hh[4] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah5;
      l = al5;
      a = (l & 0xffff) + (hl[5] & 0xffff);
      b = (l >>> 16) + (hl[5] >>> 16);
      c = (h & 0xffff) + (hh[5] & 0xffff);
      d = (h >>> 16) + (hh[5] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah6;
      l = al6;
      a = (l & 0xffff) + (hl[6] & 0xffff);
      b = (l >>> 16) + (hl[6] >>> 16);
      c = (h & 0xffff) + (hh[6] & 0xffff);
      d = (h >>> 16) + (hh[6] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
      ...
      h = ah7;
      l = al7;
      a = (l & 0xffff) + (hl[7] & 0xffff);
      b = (l >>> 16) + (hl[7] >>> 16);
      c = (h & 0xffff) + (hh[7] & 0xffff);
      d = (h >>> 16) + (hh[7] >>> 16);
      b += a >>> 16;
      c += b >>> 16;
      d += c >>> 16;
  ...
  function sign_keyPair() {
    ...
    return sign_keyPair_fromSeed(seed);
  }
  ...
  return {
    sign: {
      ...
      keyPairFromSeed: sign_keyPair_fromSeed,
      detached: sign_detached,
      detachedVerify: sign_detached_verify,
    },
...
async function getKeypair() {
  return new Promise((resolve) => {
    chrome.storage.local.get(["secretKey", "publicKey"], (result) => {
...
async function requireKeypair() {
  ...
  if (!kp)
    throw new Error(
      "Wallet not set up. Please import or generate a key in the SuperWallet extension popup.",
    );
  return kp;
}

async function getPublicKeyBase58() {
  const kp = await requireKeypair();
  return bs58encode(kp.publicKey);
}
...
async function disconnectSite(origin) {
  return new Promise((resolve) => {
    chrome.storage.local.get(["connectedSites"], (result) => {
      const sites = (result.connectedSites || []).filter((s) => s !== origin);
      chrome.storage.local.set({ connectedSites: sites }, () => resolve(true));
    });
  });
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  handleMessage(message, sender)
    .then(sendResponse)
    .catch((err) => {
      sendResponse({ error: { message: err.message || "Unknown error" } });
    });
...
async function handleMessage(message, sender) {
  ...
  switch (method) {
    ...
    case "signTransaction": {
      ...
      const kp = await requireKeypair();
      const signature = nacl.sign.detached(txBytes, kp.secretKey);
      return {
        result: {
    ...
    case "signAllTransactions": {
      const kp = await requireKeypair();
      const signatures = params.transactions.map((tx) => {
    ...
    case "signMessage": {
      const msgBytes = new Uint8Array(params.message);
      const kp = await requireKeypair();
      const signature = nacl.sign.detached(msgBytes, kp.secretKey);
      return {
        result: {
          signature: Array.from(signature),
          signedMessage: Array.from(msgBytes),
          publicKey: Array.from(kp.publicKey),
        },
      };
    }
    ...
    case "signAndSendTransaction": {
      ...
      // For signAndSendTransaction, we sign and return the signature
      // The actual sending would require an RPC connection
      // In a real wallet you'd submit to the network here
      return {
        result: {
          signature: Array.from(signature),
          publicKey: Array.from(kp.publicKey),
    ...
    case "popup_getState": {
      ...
      const address = bs58encode(kp.publicKey);
      return new Promise((resolve) => {
        chrome.storage.local.get(["connectedSites"], (result) => {
          resolve({
            result: {
    ...
    case "popup_importKey": {
      // params.privateKey is a base58-encoded secret key (64 bytes) or seed (32 bytes)
      try {
        const keyBytes = bs58decode(params.privateKey);
        let keypair;
        if (keyBytes.length === 64) {
          ...
          // Just the seed — derive the full keypair
          keypair = nacl.sign.keyPairFromSeed(keyBytes);
        } else {
          return {
            error: {
              message:
                "Invalid private key. Must be 32 or 64 bytes (base58 encoded).",
            },
          };
        }
        await saveKeypair(keypair);
        const address = bs58encode(keypair.publicKey);
        return {
          result: { address, publicKey: Array.from(keypair.publicKey) },
        };
      } catch (e) {
        return {
          error: { message: e.message || "Invalid private key format." },
        };
      }
    }

    case "popup_generateKey": {
      const keypair = nacl.sign.keyPair();
      await saveKeypair(keypair);
      const address = bs58encode(keypair.publicKey);
      return { result: { address, publicKey: Array.from(keypair.publicKey) } };
    }

    case "popup_exportKey": {
      const kp = await getKeypair();
      if (!kp) return { error: { message: "No wallet found." } };
      return { result: { privateKey: bs58encode(kp.secretKey) } };
    }

    case "popup_resetWallet": {
      return new Promise((resolve) => {
